package cn.jw.otb.ab.to.zone_star_order_cancel_daily
  ...
  override def clean(sc: SparkContext, config: Config, prevStepRDD: Option[RDD[String]]): Option[RDD[String]] = {
    Try {
      ...
      val advCcDayData = advCcDayDataKv.map {
        case ((zoneId, starId, liveDt, advCcDay), (rns, rev)) =>
          ((zoneId, starId, liveDt), (advCcDay, rns, rev))
      }.groupByKey.flatMap {
      ...
      val result = advCcDayData.reduceByKey {
        (x, y) => (x._1 + y._1, x._2 + y._2)
      }.map {
        case ((zoneId, starId, liveDt, advCcDay), total) =>
          val rns = total._1
          val rev = total._2
          val remark = ""
          s"$zoneId#$starId#$remark#$liveDt#$advCcDay#$rns#$rev"
      }
