package cn.jw.otb.ab.to.city_zone_order_pickup_daily_Acc
...
class Cleaner extends CleanerAssembly with Serializable {
  ...
  override def clean(sc: SparkContext, config: Config, prevStepRDD: Option[RDD[String]]): Option[RDD[String]] = {
    Try {
      ...
      val pickupDailyOrderPath = if (testOpen) pkDailyOrderDir else addDtPath(pkDailyOrderDir, dt)
      val outputDir = config.getString("dist-dir")

      val pkDailyOrderOriginal = sc.textFile(pickupDailyOrderPath).map(_.split(fieldSeparator))
