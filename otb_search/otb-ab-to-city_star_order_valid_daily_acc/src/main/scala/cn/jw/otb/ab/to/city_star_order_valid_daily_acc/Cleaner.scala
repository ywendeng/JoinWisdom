package cn.jw.otb.ab.to.city_star_order_valid_daily_acc
...
class Cleaner extends CleanerAssembly with Serializable {
  ...
  override def clean(sc: SparkContext, config: Config, prevStepRDD: Option[RDD[String]]): Option[RDD[String]] = {
    Try {
      ...
      val validDailyOrderPath = if (testOpen) validDailyOrderDir else addDtPath(validDailyOrderDir, dt)
      val outputDir = config.getString("dist-dir")

      val validDailyOrderOriginal = sc.textFile(validDailyOrderPath).map(_.split(fieldSeparator))
