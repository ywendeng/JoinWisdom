package cn.jw.otb.to.city_zone_order_valid_daily
    ...
    Try {
      ...
      val advBkDayDataKv = areaOrderFilterCancel.map {
        line =>
          ...
          val zoneId = line(colIndexAreaOrderBkDaily("zone_id"))
      ...
      val result = advBkDayData.reduceByKey {
        (x, y) => (x._1 + y._1, x._2 + y._2)
      }.map {
        case ((cityId, zoneId, liveDt, advBkDay), total) =>
          val rns = total._1
          val rev = total._2
          val remark = ""
