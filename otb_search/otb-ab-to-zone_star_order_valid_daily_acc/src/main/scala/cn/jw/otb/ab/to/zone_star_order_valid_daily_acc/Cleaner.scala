package cn.jw.otb.ab.to.zone_star_order_valid_daily_acc
...
class Cleaner extends CleanerAssembly with Serializable {
  ...
  override def clean(sc: SparkContext, config: Config, prevStepRDD: Option[RDD[String]]): Option[RDD[String]] = {
    Try {
      ...
      val validDailyOrderDir = config.getString("validDaily-order-dir")
      val dt = config.getString("dt")
      val testOpen = config.getBoolean("test-open")
      val validDailyPath = if (testOpen) validDailyOrderDir else addDtPath(validDailyOrderDir, dt)
      val outputDir = config.getString("dist-dir")

      val validDailyOrderOriginal = sc.textFile(validDailyPath).map(_.split(fieldSeparator))

      val validDailyOrderData = validDailyOrderOriginal.map {
        line =>
          ...
          ((zoneId,starId, liveDt), (advBkDays,rns, rev))
      }

     val initialResult = validDailyOrderData.groupByKey.flatMap{
       case(key,value)=>
         val starId = key._2
         val zoneId = key._1
         val liveDt = key._3
